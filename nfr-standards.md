...
| Information disclosure | **Closed 2026-07-12** — a role with no Permission row on a Machine is now denied by default (CAP-P05), replacing the old allow-by-default read; search/list still leaks are the remaining open surface | Deny-by-default reads (CAP-P05, implemented); RLS; permission-trimmed queries |
...
- **Timezone convention (added 2026-08-23, from CAP-X09's design-review closure)** — store every instant as UTC (`TIMESTAMPTZ`, already the case for every timestamp column in this schema: `created_at`/`updated_at`/`performed_at`). Two separate timezone concerns exist and must not collapse into one field: a **display** timezone (per-user, cosmetic — converts an already-known instant for whoever is looking, resolved client-side or from a User profile field, never stored as a converted value) and an **operational** timezone (per-entity/location — anchors a *business-logic* determination, e.g. "is this branch's day already closed," CAP-C11-style period boundaries, to where the business event happened, not to the server or the viewer). `today`/`now` (CAP-A02) resolve in UTC/server time by default; CAP-A02's own registry row names the proposed `timezone_field` mechanism (mirrors CAP-F17's `currency_field`) for the operational case once a real record needs it — not built yet, no case forces it. A displayed timestamp is a rendering concern and never needs a new capability to get right; a business-logic date/time comparison anchored to a specific record's own context does.
