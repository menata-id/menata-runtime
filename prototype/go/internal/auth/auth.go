// Package auth holds CAP-X02's cryptographic primitives -- password
// hashing, session bearer tokens, and CSRF tokens. Deliberately small: no
// session/user lookups here (that's store.SessionStore), no HTTP handling
// (that's cmd/server/main.go's middleware and internal/handler) -- just the
// crypto a real authentication system needs, isolated so every other layer
