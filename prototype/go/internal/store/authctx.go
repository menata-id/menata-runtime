...
// CAP-O01 per-Application role assignments (application_id -> role, loaded
// once here rather than re-queried at every permission check), and the
// session's CSRF token (compared against the submitted form field by the
// CSRF middleware, and echoed into every rendered form via
