#!/usr/bin/env bash
# Local, zero-cost CI gate for the Go prototype -- no GitHub Actions.
#
# Why this exists instead of a GitHub Actions workflow: this repo runs on a
# free GitHub plan with its Actions minutes already exhausted (owner decision,
# 2026-08-29). This gives the same practical guarantee -- nothing broken
# reaches `main` -- without needing GitHub's infrastructure at all. See
# ../../roadmap.md item 12 and ../../benchmarks/025-architecture-worldclass-audit.md
# for the audit finding this closes.
#
...
# schema on a throwaway server port -- never touches the shared dev database
# (menata_runtime) or the shared PORT=4000 dev deployment
# (see ../CLAUDE.md's "isolated-schema test server" pattern, which this
# automates). Safe to run repeatedly and tears itself down on exit, success
# or failure.
#
# Usage: ./scripts/local-ci.sh          (from prototype/go/, or via `make local-ci`)
...
BASE_DB_URL="${BASE_DB_URL:-postgres://postgres:password@localhost:5432/menata_runtime?sslmode=disable}"
case "$BASE_DB_URL" in
