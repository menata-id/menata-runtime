...
// legitimately distinct dependencies. Requires DATABASE_URL seeded with
// 001+004+050.
func TestGroupByMachineAgainstApprovalCase(t *testing.T) {
	...
	if groups[0].MachineID != "mch_approval_document" {
		t.Errorf("MachineID = %q, want mch_approval_document", groups[0].MachineID)
	}
	if len(groups[0].Nodes) != 3 {
		t.Fatalf("len(Nodes) = %d, want 3 (vw_ad_form, vw_ad_all, vw_ad_pending)", len(groups[0].Nodes))
	}
}
...
// real duplicate) -- naive=4, deduplicated=3.
func TestBuildExecutionPlanAgainstApprovalCase(t *testing.T) {
	dag := buildApprovalDocumentDAG(t)
	plan := composable.BuildExecutionPlan(dag)

	if plan.NaiveQueryCount != 4 {
		t.Errorf("NaiveQueryCount = %d, want 4", plan.NaiveQueryCount)
	}
	if plan.DeduplicatedQueryCount != 3 {
		t.Errorf("DeduplicatedQueryCount = %d, want 3", plan.DeduplicatedQueryCount)
	}
