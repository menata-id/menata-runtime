...
// multipart body, so any selected file was silently dropped). Where the
// bytes actually land is h.storage's own concern (internal/storage,
// LocalDisk on day one, behavior-identical to prototype/go's own uploads/
// directory) -- this file only decides WHAT to store (the image pipeline
// below) and WHEN (on a real multipart upload), never how the key maps to
// a filesystem path.
...
func (h *Handler) processFileUploads(r *http.Request, machine *model.Machine) (map[string]string, error) {
	...
	for _, f := range machine.Fields {
		...
		key, err := h.storage.Put(data, contentType)
		if err != nil {
...
// entropy, generated by h.storage.Put via auth.NewToken), so this
// deliberately serves without a separate per-request permission check, the
// same trust boundary this codebase already gives a session or CSRF token.
// Path-traversal protection is h.storage's own concern (LocalDisk.Get
// strips the key to its base name before touching the filesystem).
func (h *Handler) ServeFile(w http.ResponseWriter, r *http.Request) {
	key := chi.URLParam(r, "key")
	data, contentType, err := h.storage.Get(key)
	if err != nil {
		http.NotFound(w, r)
		return
	}
