...
-- Safe only once every later migration's own down has already run (024 down
-- through 003 down) -- every other table that relies on gen_random_uuid()
-- defaults is gone by the time this file's own down runs, goose applying
-- downs in reverse version order.
