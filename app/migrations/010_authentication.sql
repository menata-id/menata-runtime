...
ALTER TABLE users DROP COLUMN IF EXISTS workspace_role;
-- Best-effort only: the up migration DROPPED the original single-valued
-- `role` column outright (replaced by the two-tier workspace_role +
-- user_application_roles model, see this file's own header) -- those
-- per-row values are gone and cannot be reconstructed. This restores the
-- column's shape so the schema matches migrations/002 again, not its data.
