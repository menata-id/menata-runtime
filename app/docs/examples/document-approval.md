...
| Approver picker narrowed to a Group | `CAP-F23` — `fld_as_approver.options.restrict_to_group` | `seeds/038_group_approver_lab.sql` |
...
machine:
  ...
  fields:
    ...
    - { id: fld_as_approver, name: Approver, type: user, required: true,
        # CAP-F23 — see §2 below, this is the Group tie-in
        options: { restrict_to_group: "Document Approvers" } }
  ...
  permissions:
    - { role: Approver, events: [evt_as_approve, evt_as_reject] }
```

Real source: `app/seeds/004_approval.sql`. Full field/constraint/view lists there — this section
keeps only what matters for the composed app the mockups show.
...
Once the Group exists with that exact name, `fld_as_approver`'s `restrict_to_group` (declared in
metadata, §1 above) picks it up automatically — no reload needed, `GroupStore.GetByName` resolves
by name at request time. If the name doesn't resolve to a real Group, the picker degrades
gracefully to its unrestricted candidate list (still narrowed by role) — this is a UX narrowing,
never the actual authorization boundary; the Permission's own role check is.

## 3. Progress and signature position — each a full page, and both inline together
...
## 4. Reproducing this app from scratch

In order, against a fresh database (`make migrate-up` first):
...
3. `seeds/035_pdf_signature_lab.sql` + `seeds/036_coord_placement_lab.sql` — signature fields and
   the `coord_placement` View, §3 above.
4. `seeds/037_decision_stepper_lab.sql` — §3's other full-page stepper.
5. `seeds/038_group_approver_lab.sql` — §2's `restrict_to_group` declaration.
6. `seeds/042_inline_view_composition.sql` — §3's inline composition of both.
7. Through the running app, as a workspace Admin: create the "Document Approvers" Group, add
   members (§2) — this step has no seed file, on purpose.

Every metadata-only step (1–6) takes effect on the next server start, or immediately via
`POST /{ws}/admin/reload` (`CAP-X04`) with no restart at all — verified live, both ways, building
this exact reference (`capability-registry.md`'s `CAP-V20` row has the dated proof).
