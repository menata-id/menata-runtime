...
4. **`internal/testing` package** — scaffolding to build before the "real coverage push" gap can
   be closed at all.
5. **ADR index** (`app/docs/decisions/README.md`) — a five-minute documentation fix, no code
   change.
6. **Actor-first convention**, written into `internal/permission/doc.go` and checked by grep —
   formalizes an existing informal pattern before more packages are ported.
