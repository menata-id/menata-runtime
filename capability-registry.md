...
| CAP-A03 | `notify` to role | ✅ | Case 1 | — | | conformance T31. **Implemented 2026-07-11:** `Executor.doNotify` writes a real `notifications` row (CAP-A10) for every `notify: {role: ...}` action, not just a slog line. "To role" still means this prototype's only identity concept — a session's role cookie — so the delivery target is every session logged in as that role, not a specific person, the same identity-is-role caveat CAP-A02's `current_user` already carries |
...
| 6 | CAP-P02, CAP-P05, CAP-E05 | Record/CRUD-level permission + system events — **next up** |
