...
| CR-27 | Grammar-area decision for `Dataset` (new Grammar area `D`, alongside `F/E/A/C/P/V/R/X/I/O`, vs. folding under `View`) | **RESOLVED 2026-09-11** — owner decision: new Grammar area `D` (Data), consistent with the Domain/Data/Experience plane split `004`/`006`/`007` commit to. `CAP-V22`/`CAP-V23` migrate out of `V` into `D` (renumbered `CAP-D01`/`CAP-D02`) with an append-note on the old rows, per the registry's ratchet rule. Registry/lifecycle/NFR edits tracked separately. | P0 |
