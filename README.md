...
| [composable-runtime-roadmap-phase0-documentation-alignment.md](composable-runtime-roadmap-phase0-documentation-alignment.md) | The detailed DOC-01–DOC-10 backlog and itemized exit gate for `composable-runtime-roadmap.md`'s Phase 0 — **all ten items resolved 2026-09-11**; Phase 0 is closed |
...
| [prototype/objectstack/README.md](prototype/objectstack/README.md) — **lives under `prototype/`, not `benchmarks/`, by explicit owner instruction** | Study 37 — ObjectStack (`objectstack-ai/objectstack`, a TypeScript/Zod metadata runtime with a microkernel, "apps small enough for AI to hold whole") studied from its own source tree (commit `ac76425f`, 131 ADRs, 864-file Zod spec) and compared area-by-area — field types, views, workflow/automation, approvals, permissions, analytics, backend architecture and folder structure — against `capability-registry.md` v0.54. 31 gaps found, 23 recommendations (R1–R23) framed by the owner's "flexible + powerful" brief: adopt the *ideas* (CEL-style expression layer, semantic `dataset`, scope-depth permissions over a unit tree, chart widget, approval enrichments, outbound webhook action, governed MCP surface) and none of the *architecture* (plugin kernel, TS authoring, SPA, code hooks, DAG engine, table-per-object). Also names where Menata is ahead (metadata-only, conformance ratchet, Process Overlay, business-calendar SLA, change policy) and ObjectStack's own "declared ≠ enforced" history as the reason to keep the ratchet. **No capability admitted** — 14 candidate IDs named, owner decision pending |
| [prototype/symfony/README.md](prototype/symfony/README.md) — **lives under `prototype/`, not `benchmarks/`, by the owner's own suggestion** | Study 39 — reconciles an owner-supplied essay (an AI session's own `app/`-vs-Symfony analysis, twelve gap areas) against Study 37/38 and the live registry. Symfony has no metadata layer, so this compares *infrastructure* primitives, not capability breadth. Verdict: every real gap the essay names (validation/expression, API+OpenAPI, permission scope depth + approver resolution, observability trace, metadata versioning) is already registered from Study 37 (`CAP-C13` ✅, `CAP-X07` T2, `CAP-P08`/`CAP-P09`/`CAP-O12`, `CAP-I04`, `CAP-X17`) — a third independent source on the same short list. A DI/Service Container and a Bundle/plugin kernel are both rejected as runtime mechanisms, same reasoning as Study 37 §2's rejection of ObjectStack's microkernel, reinforced by Symfony's own field lesson (bundles are for cross-application sharing, not app code, since Symfony 4/Flex — verified against current `symfony.com/doc`). Composable View, generic events, and form abstraction are all already solved by Menata's own metadata grammar, which the essay independently re-derives without recognizing it exists. **No capability admitted, no new candidate ID** — one new terrain connection recorded (`CAP-P07` public access ↔ the already-named missing rate-limiting middleware) for a future admission test |
| [benchmarks/030-ui-subcomponent-decomposition-criteria.md](benchmarks/030-ui-subcomponent-decomposition-criteria.md) | Study 40 — owner-requested continuation of Study 38 once Case 3 and Case 19 mockups both reached "complete": re-runs Study 38's cluster method against Case 19's 9 new mockups, adds a decomposition-criteria section grounded in 5 real (non-metadata) design systems (Atomic Design, Shopify Polaris, GitHub Primer, Atlassian Design System, Material Design 3, synthesized into a presentation-layer analogue of `capability-lifecycle.md` §2's own A1–A5 test), and maps every cluster onto whether Runtime Metadata can actually invoke it today. Three Study 38 clusters (Stat Tile, Activity Feed, Two-column layout) gain cross-domain confirming evidence; one new automatic mechanism found (the Board/Timeline/Calendar/Dashboard view-switcher, already covered by ADR-008/`CAP-O03` Tier 3, just missing from the mockups themselves — fixed same session); one real, three-times-confirmed gap named (Activity Feed / `record_events` timeline View, reinforcing R28/`CAP-R04`). No capability admitted, no `guides/writing-runtime-metadata.md` change (documents shipped schema only) |
...
| [prototype/objectstack/](prototype/objectstack/) | **Not a metadata proof** — Study 37's comparator study of ObjectStack, a peer metadata runtime (added 2026-09-07, placed here by owner instruction); unscored, produces a gap list and candidate capabilities, listed in Tier 4 above |
| [prototype/symfony/](prototype/symfony/) | **Not a metadata proof** — Study 39's comparator study of Symfony, a general-purpose framework with no metadata layer (added 2026-09-07, placed here by owner suggestion); unscored, reconciles an owner-supplied essay against Study 37/38 and the live registry, listed in Tier 4 above |
