...
- **`benchmarks/` — numbered `000`–`027`** — evidence studies, numbered by production order (Study 1 → `000`, Study 2 → `001`, …), not a required reading sequence. Grows as new studies land; check `ls benchmarks/` for the current top of the range rather than trusting this exact number to stay current.
