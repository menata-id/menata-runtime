...
## Study 15 — Field Modeling Decision Framework ✅ done, second-pass corrected (2026-07-05)
...
- [x] Retrofit calibration against every field in Cases 1–10 — reproduces almost every prior ad hoc choice correctly; a second, adversarial pass corrected two initial miscategorizations (see below)
...
- [x] `capability-registry.md` refined — CAP-F13 gains an explicit **three**-target-flavor scope note (workspace Machine, built-in identity, built-in File/Document), CAP-F05/F06/F17 gain long-term resolution notes, CAP-O02 gains two independent confirming cases (`Equipment`, `Currency`)
...
- **Predicted, not yet registered:** a future `Quantity` field (Case 5 — Inventory) would be a third instance of the same pattern (count + Unit of Measure). Deliberately not added as a capability yet — no case evidence exists until Case 5 is written, honoring the admission test's dual-evidence rule rather than registering on prediction alone.
...
- The calibration discipline itself was validated by surviving a second, adversarial pass — a framework that only gets checked once is a framework that hasn't really been checked.
