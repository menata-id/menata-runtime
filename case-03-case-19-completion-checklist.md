...
4. **Stage A — small, low-risk, classic-only fixes, independent of each other:**
   ...
   ~~Inline PDF preview~~ **done, 2026-09-12 (17r)** — sized correctly as a small, backend-free
   change; reused `isPDFPreview` (already proven by CAP-V21's own `coordplace.templ`) rather than
   inventing a second file-type detection. Remaining Stage A item, not yet started: Board
   drag-reorder within one lane (Case 19 screen 1, client-side + a `sort_order` write, same shape
   `MoveToLane`/`Move` already use).
...
- Approval inbox (screen 3) — SLA wiring and inline PDF preview both done, 2026-09-12 (17r);
  `DecisionStepper` progress still has no composable representation.
